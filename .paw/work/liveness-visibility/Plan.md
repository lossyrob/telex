...
- Define terminal outcomes from a single source of truth (`WaiterOutcome` enum or shared constants with serde/value tests), not inline strings. Member-recorded outcomes are `message` (exit `0`), `idle-timeout` (exit `2`), `presence-ended` (exit `5`, detail disambiguates `session-end`, `station-stop`, `idle-ttl-reap`, `reset`, or `watch-pid-death`), and `abnormal-exit` (no telex exit code when the waiter process dies before daemon response).
