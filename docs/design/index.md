...
DeregisterSession proof (canonical: **sessionEnd removal proof** — now session-end + explicit membership via `Detach`/`Ack`), the Status freeze line, and crash durability — are resolved in
