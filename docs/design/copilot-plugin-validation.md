...
| GitHub-fetch install (production path) | Real `owner/repo#ref` fetch: `copilot plugin marketplace add lossyrob/telex#feature/harness-skill-layout` + `plugin install telex@telex` against a fresh isolated `--config-dir`, plus a `copilot -p 'Respond OK'` hook-log smoke. See the transcript captured in the PR. |
