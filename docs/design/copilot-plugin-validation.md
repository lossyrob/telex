...
| Copilot App turn-idle does not end attendance | A real App `sessionEnd` payload uses `reason: complete` after a turn while the bridge remains alive. `copilot session-end` preserves membership and turn-guard state, refreshes every session member with the bridge host PID as its watch anchor, and still honors `user_exit`/other terminal reasons or a missing/stale bridge. The host is stable across extension reloads and its death reaps a true App or one-shot CLI exit through the existing watch-pid path. |
| Busy deferral remains healthy in status | The daemon reports `push_delivery: deferred` and `station_health: attended_push` when the bridge answers `deferred_until_idle`; a same-timestamp transport failure wins the health tie deterministically. |
