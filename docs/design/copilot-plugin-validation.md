...
| Operator compatibility review is release-coupled | Stable-path `compatibility.json` records the package/plugin versions, Operator Station extension, identity derivation, workflow signature, accepted send receipts, reconciliation outcomes, and required runtime capabilities. Plugin and release contract tests require an explicit fixture update on version drift. |
