...
| Hooks are contributed by a real plugin manifest | `copilot/plugin/plugin.json` declares `hooks.json`; `copilot/plugin/hooks.json` declares `sessionEnd` and `agentStop` command hooks. `sessionEnd` and `turn-guard` invoke hidden Rust adapters directly. Only the drain entry crosses small plugin-owned PowerShell/POSIX launchers, which forward stdin to the binary-owned adapter and translate pre-dispatch failure into Copilot hook JSON. |
| Same-semver binary skew is diagnosable | `telex --version` includes a source build identifier, `telex --json version` exposes it as `version.build_id`, and versioned-install manifests retain it. New binaries accept older version JSON/manifests without that additive field as `unknown`. |
...
| Drain hook is dedicated and independently gated | `hooks.json` `agentStop` runs both `copilot turn-guard` and the drain launcher. `TELEX_COPILOT_DRAIN=off|0|false` skips launch and allows. Once `copilot drain` starts, its bounded runtime/daemon handling remains fail-open (exit 0). A missing/stale PATH binary or other nonzero pre-dispatch failure instead becomes one actionable `block` decision; the launcher itself exits 0 so Copilot processes that JSON. `DrainDeferred` is admin-capped (`drain_deferred_requires_admin_cap`). |
...
`copilot/plugin/` (the marketplace plugin root: `plugin.json`, `hooks.json`, drain hook
launchers, and `skills/telex/SKILL.md`). `.github/plugin/marketplace.json` sets the plugin
...
| Installed skill resolves from the nested plugin root | The installed plugin tree contains `plugin.json`, `hooks.json`, `drain-hook.ps1`, `drain-hook.sh`, and `skills/telex/SKILL.md`, all copied from `copilot/plugin/`. |
| Installed plugin is lean (no embedded sources shipped) | `copilot/COPILOT.md` and `copilot/bridge/` are siblings of `copilot/plugin/`, so they are NOT copied into the installed plugin. The shipped launchers remain small transport/error adapters; binary-owned workflow content is not duplicated. |
