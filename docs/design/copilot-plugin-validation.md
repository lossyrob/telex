...
| Bounded-timeout notification heartbeat is omitted | A local spike found sync/agent-read shell completions already carry stdout in context, while the notification hook payload exposes only notification metadata and not a stable `--out-dir` path. `hooks.json` intentionally does not install a notification hook; Tier-A relies on the default-on `agentStop` guard. |
